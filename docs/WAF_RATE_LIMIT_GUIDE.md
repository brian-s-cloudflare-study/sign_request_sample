...
WAF Custom Rule과 Rate Limiting�� 설정하는 방법을 안내���니다.
...
1. **Cloudflare Dashboard** → 해당 도메�� → **Security** → **WAF** → **Custom rules**
2. **Create rule** 클릭
3. 아래 설정 입력:
...
캘린더 날짜를 빠르게 반복 클릭하는 ��의 남용을 방지합니다.
...
```bash
# 빠르�� 반복 요청 → 일정 횟수 후 429 또는 Managed Challenge
...
| 키 | 동작 |
|---|---|
| Site Key: `1x00000000000000000000AA` | 항상 성공 (보��는 위젯) |
...
| Turnstile 위젯 �� 뜸 | Site Key 누락/불일치 | TURNSTILE_SITE_KEY 확인 |
